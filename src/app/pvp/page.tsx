...
import { CategoryLiveChip } from "@/components/pvp/PracticeLive";
...
export default async function PvpHubPage() {
      ...
      <header className="relative isolate overflow-hidden border-b border-border pb-14 pt-36 sm:pt-44">
        <div className="relative container-base max-w-5xl">
          ...
          <div className="mt-9 grid gap-8 lg:grid-cols-12 lg:items-end">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-px border border-border bg-border rounded-lg overflow-hidden lg:col-span-5 lg:col-start-8">
              {[
                ["Online", pulse ? String(pulse.onlinePlayers) : "—"],
                ["In match", pulse ? String(pulse.inFight) : "—"],
                ["In queue", pulse ? String(pulse.inQueue) : "—"],
      ...
      <section className="relative container-base max-w-5xl pb-28 pt-14">
        <ul className="space-y-4">
          {PVP_CATEGORIES.map((cat) => {
            ...
            return (
              <li key={cat.slug}>
                <Link href={`/pvp/${cat.slug}`} className="group block">
